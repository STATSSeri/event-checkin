    ...
    <div className="min-h-screen p-4 md:p-8 max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div className="text-center mb-4">
          <span className="text-4xl font-bold text-green-600">{checkedInCount}</span>
        ...
        <div className="text-center text-sm text-gray-500 mt-2">{progressPercent}%</div>
      </div>
      ...
      {sortedGuests.length === 0 ? (
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          {sortedGuests.map((g) => (
            <div key={g.id} className="flex items-center gap-3 px-4 py-3 border-b border-gray-50">
              {/* ステータスドット */}
              <div
              ...
              <div className="flex-1 min-w-0">
                ...
                {g.organization && (
                  <div className="text-xs text-gray-400 truncate">{g.organization}</div>
                )}
              </div>
              ...
              {/* 取消ボタン */}
              {g.status === 'checked_in' && (
                <button onClick={() => handleUndoCheckin(g)}
