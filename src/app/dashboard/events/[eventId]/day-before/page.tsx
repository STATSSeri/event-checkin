...
export default function DayBeforePage() {
  ...
  const fetchData = useCallback(async () => {
    // 出席確定者のみ取得（attending）。チェックイン済みは除外
  ...
  const handleSend = async () => {
    ...
    if (!confirm(
      `${selected.size}名にご来場前のご案内（QRコード付き）を送信します。\n\n` +
      `※QRコードは出欠回答時にお送りしたものと同一です。\n` +
      `送信を実行しますか？`
    )) return;
    ...
    try {
      const res = await fetch('/api/send-day-before', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-4xl mx-auto">
      ...
      <h1 className="text-xl font-bold text-gray-800 mb-1">前日リマインド送信</h1>
      <p className="text-sm text-gray-500 mb-1">{event?.name}</p>
      <p className="text-xs text-gray-500 mb-2">
        出席確定者にQRコード付きで再度ご案内を送ります
      </p>

      {/* 重要な注意書き */}
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-6 text-xs text-amber-800 leading-relaxed">
        <strong>📌 ご注意</strong>
        <ul className="mt-1 ml-4 list-disc space-y-0.5">
          <li>QRコードは <strong>出欠回答時にお送りした初回QRと完全に同一</strong> のものが送られます</li>
          <li>同じQRが2通届くことになりますが、どちらをご提示いただいても受付できます</li>
          <li>すでに入場済みのゲストは対象から除外されます</li>
        </ul>
      </div>
      ...
      {guests.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-8 text-center">
          <p className="text-gray-500">出席確定のゲストがいません</p>
          <p className="text-xs text-gray-400 mt-1">
            出席回答済み・未入場のゲストが対象になります
          </p>
        </div>
      ) : (
        <>
          <div className="flex items-center justify-between bg-white rounded-lg shadow-sm p-3 mb-4">
            ...
            <button
              ...
              className="px-4 py-2 text-sm bg-amber-700 text-white rounded-md hover:bg-amber-800 disabled:opacity-50"
            >
              {sending ? '送信中...' : `${selected.size}名にご案内送信`}
            </button>
          </div>
                ...
                <div className="text-right hidden md:block">
                  ...
                  {g.qr_sent_at && (
                    <div className="text-xs text-gray-400">
                      初回QR送信:{' '}
                      {new Date(g.qr_sent_at).toLocaleDateString('ja-JP')}
                    </div>
                  )}
                </div>
              </label>
